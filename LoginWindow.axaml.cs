...
public partial class LoginWindow : Window
{
    ...
    private void LoginAttempt(object? sender, RoutedEventArgs e)
    {
        try
        {
            ...
            string sql1 = "SELECT login, password, role_id FROM us WHERE login = '" + LoginTextBox.Text +
                          "' AND password = '" +
                          hashedPassword + "' AND role_id = '1'";
            conn = new MySqlConnection(connString);
            conn.Open();
            string sql2 = "SELECT login, password, role_id FROM us WHERE login = '" + LoginTextBox.Text +
                          "' AND password = '" +
